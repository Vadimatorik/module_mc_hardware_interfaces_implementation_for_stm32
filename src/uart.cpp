...
mc_interfaces::res uart::reinit (uint32_t numberCfg) {
...
mc_interfaces::res uart::on (void) {
...
mc_interfaces::res uart::tx (const uint8_t *const txArray,
                                           uint16_t len,
                                           uint32_t timeout_ms) {
...
mc_interfaces::res uart::get_byte (uint8_t *buf) {
    if (this->u.gState == HAL_UART_STATE_RESET) {
        return mc_interfaces::res::errInit;
    }
...
void uart::dma_irq_handler (void) {
